...
1. **A Connection** (formerly *Client*) — with an `awsUserName`, created in the dashboard or via API
...
<Tip>
`clientName` is the connection's server-generated `awsUserName` slug, not its display name. Resolve it with `connections.getByName('Acme').awsUserName`. Output resolution: `outputFormat` → extension of `outputFilename` → input filename (defaults to JSON).
</Tip>
...
<Note>
**Need a column that's always the same value?** Mark the field as an *exact
value* field on the **schema** (`"exactValue": true`), then set the constant on
the pipeline via the mapping's `staticValue`. FileFeed writes that constant into
the column on every delivered row, so you don't have to include it in each
upload payload. The value is per pipeline, so connections sharing a schema can
each send a different fixed value. In the dashboard, toggle **Exact value** on
the field in the schema editor, then enter the value on the outbound pipeline.
</Note>
