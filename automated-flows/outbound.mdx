...
const init = await filefeed.outbound.initUpload({
  clientName: 'acme-corp',           // awsUserName of the connection (field kept for back-compat)
...
- [ ] Create Connection with SFTP credentials (provides the `clientName` / `awsUserName` — names kept for back-compat)
