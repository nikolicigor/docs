...
<Note>
**Need a column that's always the same value?** Add a *static value* mapping to
the pipeline — `{ "targetField": "sourceSystem", "staticValue": "ACME" }` (no
`sourceField`). FileFeed writes that constant into the column on every delivered
row, so you don't have to include it in each upload payload. In the dashboard,
use the **Static columns** section of the outbound pipeline editor.
</Note>
