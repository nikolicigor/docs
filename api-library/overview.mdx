...
<AccordionGroup>
  <Accordion icon="users" title="clients">
    ...
    | retrieve | `retrieve(id)` | Get a client by id. |
    | getByName | `getByName(name)` | Get a client by name (unique per workspace). Resolves to the client or `undefined`. |
    ...
    **Common workflows**
    ...
    - Resolve a client's `awsUserName` (the `clientName` used by outbound uploads) from its human-readable name

    ```ts
    const client = await filefeed.clients.create({ name: 'Acme' });
    const ok = await filefeed.clients.testConnection(client.id);

    // Look a connection up by name, then read its awsUserName (the value
    // outbound uploads expect as `clientName`). Credentials are never returned.
    const conn = await filefeed.connections.getByName('Acme');
    if (conn) console.log(conn.awsUserName, conn.sftpUsername);
    ```
  </Accordion>
  ...
  <Accordion icon="arrow-up-from-bracket" title="outbound">
    ...
    **Common workflows**
    ...
    `clientName` is the connection's `awsUserName` (resolve it with `connections.getByName(name).awsUserName`).
