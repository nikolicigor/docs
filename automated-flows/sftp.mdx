...
{
  "connectionId": "c4b3f495-5dfc-4a91-b604-a8e66ab4a220",
  "name": "acme_logistics",
...
<Note>
**Exact values (static fields).** Mark a field as an *exact value* field on the
**schema** (set `"exactValue": true` on the property, or toggle **Exact value**
in the schema editor). Each pipeline using that schema then supplies the
constant via the mapping's `staticValue` — so different connections sharing a
schema can each send a different fixed value (e.g. a partner-specific
source-system id). When set, `staticValue` is written into the column for
**every** row and `source`/`transform` are ignored. `staticValue` accepts a
string, number, boolean, or `null`. Works for both inbound and outbound. Because
the flag lives on the schema, a field marked exact-value is static for *every*
pipeline on that schema (only the value varies per pipeline).
</Note>
